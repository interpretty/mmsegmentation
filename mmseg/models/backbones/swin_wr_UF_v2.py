    ...
    def forward(self, x, mask=None, global_tuple=None):
        ...
        if len(global_tuple) == 1:
            ...
            if attn3 is None:
                ...
                mask_diag[indices, indices] = 0
                # 点乘置零
                attn3 = attn_rpbp_diag.reshape(B, self.num_heads, L, 4, 4, 4) * mask_diag

            global_tuple.append(attn3)

        else:
            relative_position_bias, attn1, attn2, attn3 = global_tuple
            _, _, L, _ = attn1.shape
            ...
            if attn3 is not None:
                q = q.view(B, self.num_heads, ws // 8, 2, ws // 8, 2 * 2 * 2, 4, -1).permute(
                    0, 1, 2, 4, 3, 5, 6, 7).reshape(
                    B, self.num_heads, ws // 8 * ws // 8 * 2 * 2 * 2 * 2, 4, -1).contiguous()
                k = k.view(B, self.num_heads, ws // 8, 2, ws // 8, 2 * 2 * 2, 4, -1).permute(
                    0, 1, 2, 4, 3, 5, 6, 7).reshape(
                    B, self.num_heads, ws // 8 * ws // 8 * 2 * 2 * 2 * 2, 4, -1).contiguous()
            ...
            # 三次置换
            if attn3 is not None:
                v = v.view(B, self.num_heads, ws // 8, 2, ws // 8, 2, 2 * 2, 4, -1).permute(
                    0, 1, 2, 4, 3, 5, 6, 7, 8).reshape(
                    B, self.num_heads, ws // 8 * ws // 8, 2 * 2, 2 * 2, 4, -1).contiguous()

            if attn3 is None:
                pass
            else:
                v_attn1 = torch.sum(v, dim=(3, 4, 5), keepdim=False) / (4 * 4 * 4)
                v_attn2 = torch.sum(v, dim=(4, 5), keepdim=False) / (4 * 4)
                v_attn3 = torch.sum(v, dim=5, keepdim=False) / 4
                v_diag = v.reshape(B, self.num_heads, ws // 8 * ws // 8 * 2 * 2 * 2 * 2, 4, -1)
            ...
            # # size
            # x_diag:8，8，16384，32 = 8，8，256，4，4，4，32
            # x_attn1:8,8,256,32
            # x_attn2:8,8,256,4,32
            # x_attn3:8,8,256,4,4,32

            x = x_diag.reshape(B, self.num_heads, L, 4, 4, 4, C // self.num_heads) \
                + x_attn3.reshape(B, self.num_heads, L, 4, 4, 1, C // self.num_heads) \
                + x_attn2.reshape(B, self.num_heads, L, 4, 1, 1, C // self.num_heads) \
                + x_attn1.reshape(B, self.num_heads, L, 1, 1, 1, C // self.num_heads)
            ...
            # 同时完成两次置换
            if attn3 is not None:
                x = x.view(B, self.num_heads, ws // 8, ws // 8, 2, 2, 2, 2, 2, 2, C // self.num_heads).permute(
                    0, 2, 4, 6, 8, 3, 5, 7, 9, 1, 10).reshape(B, ws * ws, -1).contiguous()
