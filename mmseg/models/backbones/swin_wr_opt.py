...
class ShiftWindowMSA1(BaseModule):
...
class ShiftWindowMSA2(BaseModule):
    """Shifted Window Multihead Self-Attention Module.
    ...
    def forward(self, query, hw_shape, sim_map):
        B, L, C = query.shape
        H, W = hw_shape
        assert L == H * W, 'input feature has wrong size'
        query = query.view(B, H, W, C)
